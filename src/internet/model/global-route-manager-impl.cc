...
//
  ...
  //依次遍历所有的节点，对每个节点，都以它为SPF树的root节点算一遍SPF树
  for (NodeList::Iterator i = NodeList::Begin (); i != listEnd; i++)
    {
      Ptr<Node> node = *i;
//
...
// route.
//
//此处时真正添加各个分立节点路由表表项的地方
//节点作为一个入参，是刚刚被加入到SPF树上的，这个节点必须有一个有效的m_root_oid，和树的根路由器到这个节点的路线的出口接口相对应，
//这个节点也必须有一个下一跳地址，和根节点到这个节点的下一跳地址相对应。
//这个节点有一个m_lsa域，里面有一些链路记录。对于每条点对点链路记录而言，m_linkData是链路的本地IP地址
void
...
// going to use this ID to discover which node it is that we're actually going
// to update.
// 最短路径有限树的根几点就是我们即将要写入实际路由信息的路由器。
// 跟这个路由器相关的Vertex拥有一个VertexId，
// 它就时路由器的id。我们将使用这个id来找到我们要更新信息的节点。
  Ipv4Address routerId = m_spfroot->GetVertexId ();
...
// to be interested in the records corresponding to point-to-point links.
//
//从节点中获取Link State Advertisement，LSA将会有一个数据
//即attached global router link records
//和离开这个节点的链路相关，我们只挑选点对点链路。
...
// the vertex <v>.
//
//遍历我们将要加入路由器的节点上的link record。确定我们明了将要加入路由表项到节点中去，节点就是SPF树的根节点
//这些表项将会由节点v所描述的节点上的点对点链路的本地端所表达！乱翻译的。
...
// which the packets should be send for forwarding.
//
//以上完成我们的工作，我们将要把路由项加入到在点对点link记录中的m_linkData域中找到的主机地址上
//在点对点链路中，这就是连接到这条link上的节点的本地ip地址。每条点对点链路都会和一个本地接口相对应，接口拥有
//IP地址，这个ip地址就是SPF树可以发送包的地址。点v（这些链路和接口相对应）有一个预先计算好的m_nextHop地址
//根节点该把包发送到这些ip地址上去。类似的，点v也有一个m_rootoif，出接口索引，用于转发包。
              Ptr<GlobalRouter> router = node->GetObject<GlobalRouter> ();
              if (router == 0)
                {
                  continue;
                  ...
                  if (outIf >= 0)
                    {
                      ...
                      //added by luodan
                      std::cout<<"\n"<<"luodan: (Route " << i << ") Node " << node->GetId () <<
                              " adding host route to " << lr->GetLinkData () <<
                              " using next hop " << nextHop <<
                              " and outgoing interface " << outIf<<std::endl;
                      std::cout<<"\n"<<"luodan: the distance from the root Node "<<node->GetId()<< " to " <<lr->GetLinkData ()<<
                    		  ": "<< v->GetDistanceFromRoot()<<std::endl;

                      //add the cost of the shortest path between the root node and the destNode
                      //the CostTable is a member of every node.
                      //find the node Id of the input SPFVertex v, then push the entry into the table
                      Ipv4Address destinationRouterId = v->GetVertexId ();
                      NodeList::Iterator i = NodeList::Begin ();
                      NodeList::Iterator listEnd = NodeList::End ();
                       for (; i != listEnd; i++)
                         {
                           Ptr<Node> tempNode = *i;
                           Ptr<GlobalRouter> tempRouter = tempNode->GetObject<GlobalRouter> ();
                           if (tempRouter == 0)
                             {
                               NS_LOG_LOGIC ("No GlobalRouter interface on node " << tempNode->GetId ());
                               continue;
                             }
                           NS_LOG_LOGIC ("Considering router " << rtr->GetRouterId ());
                           //find the node which corresponds to SPFVertex v
                           if (tempRouter->GetRouterId () == destinationRouterId)
                             {
                        	   //注意这里是要将目标节点和cost值的表项添加到根节点的map中，node是之前找出来的root node
                               node->AddPathCostTableEntry(tempNode->GetId(),v->GetDistanceFromRoot());
                               break;
                             }
                         }//added by luodan
                    }
