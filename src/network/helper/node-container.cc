...
//added by luodan
void
NodeContainer::Delete(uint32_t nodeId)
{
	uint32_t size = m_nodes.size();
	if(size == 0||nodeId >= size)
	{
		return;
	}
	bool isExist = false;
	uint32_t i = 0;
	for(std::vector<Ptr<Node> >::iterator nodeIter = m_nodes.begin();nodeIter != m_nodes.end(); nodeIter++)
	{
		if(i == nodeId)
		{
	...
	if(isExist == false)
	{
		std::cout<<"cant find the snode idn when delete"<<nodeId<<std::endl;
	}
}
} // namespace ns3
