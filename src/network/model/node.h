  ...
  /**
    ...
    * add the cost of the shortest path between the root node and the destNode
      the CostTable is a member of every node.
       //物理节点专用函数
    */
  void AddPathCostTableEntry(uint32_t destNodeId,uint32_t costValue);

  /**
    * \added by luodan
    * return the cost Table of the shortest path between  nodes
    * //物理节点专用函数
    */
  void GetPathCostTableEntry(std::map<uint32_t,uint32_t>& SPFCostTable);
...
private:
  ...
  //added by luodan
  //key:dest node ID,value:the cost between the dest node and this node
  //物理节点专用变量
  std::map<uint32_t,uint32_t> m_SPFCostTable;
};
