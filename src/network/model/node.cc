...
//物理节点专用函数
void
Node::AddPathCostTableEntry(uint32_t destNodeId,uint32_t costValue)
{
	NS_LOG_FUNCTION (this << destNodeId<<costValue);

    std::map<uint32_t,uint32_t>::iterator i = m_SPFCostTable.begin();
    for(;i != m_SPFCostTable.end(); i++)
    {
    	if(i->first == destNodeId)
    	{
    		std::cout<<"luodan: The entry of dest node "<<destNodeId<<" in node "<<this->GetId()<<" is existed."<<std::endl;
    		return;
    	}
    }
    if(i == m_SPFCostTable.end())
    {
    	m_SPFCostTable.insert(std::make_pair(destNodeId,costValue));
    	std::cout<<"luodan: New Entry in the node "<< this->GetId() <<": dest node "<<destNodeId<<" node cost "<< costValue << std::endl;
...
void
//物理节点专用函数
Node:: GetPathCostTableEntry(std::map<uint32_t,uint32_t>& SPFCostTable)
{
	NS_LOG_FUNCTION (this << "added by luodan the function Node::GetPathCostTableEntry");
	SPFCostTable.clear();
	std::map<uint32_t,uint32_t>::iterator i = m_SPFCostTable.begin();
	for(;i != m_SPFCostTable.end(); i++)
	{
		SPFCostTable.insert(std::make_pair(i->first,i->second));
		NS_LOG_LOGIC("Get the Entry in the node "<< this->GetId() <<": dest node "<< i->first<<" node cost "<< i->second);
	}
}
